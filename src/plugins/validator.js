...
class validate {
	...
	__initMethods() {
		...
		that.methods = {
			 ...
			 */
			vcode(value) {
				return that.optional(value) || /^\d{4}$/.test(value)
			},
			/**
			 * 验证交易密码
			 */
	 ...
	 * @param {Object} detail 表单数据对象
	 */
	checkParam(param, rules, detail) {
	...
	checkForm(detail) {
		...
		for (let param in this.rules) {
			// 判断是否依赖于某个字段
			dependName = this.rules[param].dependOn
			if(dependName && detail[dependName] == false) {
				continue
			}
			this.checkParam(param, this.rules[param], detail)
		}
