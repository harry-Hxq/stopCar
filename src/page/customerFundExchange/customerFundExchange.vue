<template>
    ...
    <div class="content">
        ...
        <div class="" v-show="activeStep === 2">
            <divider>3 填写兑换金额</divider>

            <group>

                <x-input :title="mapping.currencyCnName[item]" placeholder="请输入兑换金额" v-for="item in from_currency" type="text" v-model="covertObj[item]" text-align="right" ></x-input>
            </group>
            <box gap="10px 10px">
                <x-button type="primary" action-type="button" @click.native="showTradePassword">确认兑换</x-button>
            </box>
        </div>
    </div>
</template>
...
<script>
    ...
    import {cashAccount,banks,exchange} from '../../service/getData'
    ...
    export default {
        ...
        mounted() {
            this.$refs.headtop.rightUrl = '/customerFundExchangeLog?uid='+this.uid
            this.$refs.headtop.rightTitle = '兑换记录'
        },
